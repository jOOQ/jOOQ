...
final class MetaSQL {
    ...
    static final String M_ATTRIBUTES(SQLDialect dialect) {
        return M_ATTRIBUTES.get(dialect);
    }

    static final String M_IDENTITIES(SQLDialect dialect) {
        return M_IDENTITIES.get(dialect);
    }
    ...
    static {
        ...
        M_CHECKS.put(DUCKDB, "select duckdb_constraints.database_name, duckdb_constraints.schema_name, duckdb_constraints.table_name, ('CHECK_' || cast(row_number() over (partition by duckdb_constraints.database_name, duckdb_constraints.schema_name, duckdb_constraints.table_name order by duckdb_constraints.constraint_text) as varchar)) constraint_name, duckdb_constraints.constraint_text from duckdb_constraints() where (duckdb_constraints.schema_name in (cast(? as varchar)) and duckdb_constraints.constraint_type = 'CHECK') order by duckdb_constraints.database_name, duckdb_constraints.schema_name, duckdb_constraints.table_name, row_number() over (partition by duckdb_constraints.database_name, duckdb_constraints.schema_name, duckdb_constraints.table_name order by duckdb_constraints.constraint_text)");
        ...
        M_CHECKS.put(HSQLDB, "select alias_1111860.TABLE_CATALOG, alias_1111860.TABLE_SCHEMA, alias_1111860.TABLE_NAME, CONSTRAINT_NAME, cc.CHECK_CLAUSE from (INFORMATION_SCHEMA.CHECK_CONSTRAINTS as cc left outer join INFORMATION_SCHEMA.TABLE_CONSTRAINTS as alias_1111860 on (cc.CONSTRAINT_CATALOG = alias_1111860.CONSTRAINT_CATALOG and cc.CONSTRAINT_SCHEMA = alias_1111860.CONSTRAINT_SCHEMA and cc.CONSTRAINT_NAME = alias_1111860.CONSTRAINT_NAME)) where (alias_1111860.TABLE_SCHEMA in (cast(? as varchar(128))) and (alias_1111860.CONSTRAINT_NAME not like 'SYS!_CT!_%' escape '!' or replace(cc.CHECK_CLAUSE, '\"', '') not in (select (((((c.TABLE_SCHEMA || '.') || c.TABLE_NAME) || '.') || c.COLUMN_NAME) || ' IS NOT NULL') from INFORMATION_SCHEMA.COLUMNS as c where (c.TABLE_SCHEMA = alias_1111860.TABLE_SCHEMA and c.TABLE_NAME = alias_1111860.TABLE_NAME))))");
