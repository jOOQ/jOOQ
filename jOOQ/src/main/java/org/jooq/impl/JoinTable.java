...
class JoinTable extends AbstractTable<Record> implements TableOptionalOnStep<Record>, TableOnConditionStep<Record> {
    ...
    public final void accept(Context<?> ctx) {
        ...
        else if (translatedType == JOIN && ctx.family() == ACCESS) {
            keyword = "inner join";
        }
        /* [/pro] */
    ...
    private final boolean emulateCrossJoin(Context<?> context) {
        return type == CROSS_JOIN && asList(ASE).contains(context.configuration().dialect().family());
    }

    private final boolean emulateNaturalJoin(Context<?> context) {
        return type == NATURAL_JOIN && asList(ACCESS, ASE, CUBRID, DB2, HANA, INFORMIX, INGRES, SQLSERVER).contains(context.configuration().dialect().family());
    }
    ...
    private final void toSQLJoinCondition(Context<?> context) {
        if (!using.isEmpty()) {
            ...
            if (asList(ACCESS, ASE, CUBRID, DB2, H2, HANA, INFORMIX, SQLSERVER, SYBASE).contains(context.family())) {
                boolean first = true;
    ...
    /* [pro] */
    @Override
    public final TableOnStep<Record> partitionBy(Field<?>... fields) {
        return partitionBy(Utils.list(fields));
    }

    @Override
    public final TableOnStep<Record> partitionBy(Collection<? extends Field<?>> fields) {
