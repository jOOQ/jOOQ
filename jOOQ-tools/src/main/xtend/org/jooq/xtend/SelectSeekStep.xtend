 ...
 * @author Lukas Eder
 */
class SelectSeekStep extends Generators {
    ...
    def generateSelectImplImplementsSelectSeekStep() {
        val out = new StringBuilder();
        
        for (degree : (1..Constants::MAX_ROW_DEGREE)) {
            out.append('''    SelectSeekStep«degree»<R, «TN(degree)»>,
            ''');
        }
    ...
    def generateSelectSeekSteps() {
        for (degree : (1..Constants::MAX_ROW_DEGREE)) {
            ...
            write("org.jooq.SelectSeekStep" + degree, out);
        }
    }
}
