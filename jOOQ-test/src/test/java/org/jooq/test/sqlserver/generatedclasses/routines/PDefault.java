/**
 * This class is generated by jOOQ
 */
package org.jooq.test.sqlserver.generatedclasses.routines;


import java.sql.Date;

import org.jooq.Parameter;
import org.jooq.impl.AbstractRoutine;
import org.jooq.test.sqlserver.generatedclasses.Dbo;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class PDefault extends AbstractRoutine<Integer> {

	private static final long serialVersionUID = -1443407414;

	/**
	 * The parameter <code>dbo.p_default.RETURN_VALUE</code>.
	 */
	public static final Parameter<Integer> RETURN_VALUE = createParameter("RETURN_VALUE", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>dbo.p_default.p_in_number</code>.
	 */
	public static final Parameter<Integer> P_IN_NUMBER = createParameter("p_in_number", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>dbo.p_default.p_out_number</code>.
	 */
	public static final Parameter<Integer> P_OUT_NUMBER = createParameter("p_out_number", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>dbo.p_default.p_in_varchar</code>.
	 */
	public static final Parameter<String> P_IN_VARCHAR = createParameter("p_in_varchar", org.jooq.impl.SQLDataType.VARCHAR.length(10), false);

	/**
	 * The parameter <code>dbo.p_default.p_out_varchar</code>.
	 */
	public static final Parameter<String> P_OUT_VARCHAR = createParameter("p_out_varchar", org.jooq.impl.SQLDataType.VARCHAR.length(10), false);

	/**
	 * The parameter <code>dbo.p_default.p_in_date</code>.
	 */
	public static final Parameter<Date> P_IN_DATE = createParameter("p_in_date", org.jooq.impl.SQLDataType.DATE, false);

	/**
	 * The parameter <code>dbo.p_default.p_out_date</code>.
	 */
	public static final Parameter<Date> P_OUT_DATE = createParameter("p_out_date", org.jooq.impl.SQLDataType.DATE, false);

	/**
	 * Create a new routine call instance
	 */
	public PDefault() {
		super("p_default", Dbo.DBO, org.jooq.impl.SQLDataType.INTEGER);

		setReturnParameter(RETURN_VALUE);
		addInParameter(P_IN_NUMBER);
		addInOutParameter(P_OUT_NUMBER);
		addInParameter(P_IN_VARCHAR);
		addInOutParameter(P_OUT_VARCHAR);
		addInParameter(P_IN_DATE);
		addInOutParameter(P_OUT_DATE);
	}

	/**
	 * Set the <code>p_in_number</code> parameter IN value to the routine
	 */
	public void setPInNumber(Integer value) {
		setValue(P_IN_NUMBER, value);
	}

	/**
	 * Set the <code>p_out_number</code> parameter IN value to the routine
	 */
	public void setPOutNumber(Integer value) {
		setValue(P_OUT_NUMBER, value);
	}

	/**
	 * Set the <code>p_in_varchar</code> parameter IN value to the routine
	 */
	public void setPInVarchar(String value) {
		setValue(P_IN_VARCHAR, value);
	}

	/**
	 * Set the <code>p_out_varchar</code> parameter IN value to the routine
	 */
	public void setPOutVarchar(String value) {
		setValue(P_OUT_VARCHAR, value);
	}

	/**
	 * Set the <code>p_in_date</code> parameter IN value to the routine
	 */
	public void setPInDate(Date value) {
		setValue(P_IN_DATE, value);
	}

	/**
	 * Set the <code>p_out_date</code> parameter IN value to the routine
	 */
	public void setPOutDate(Date value) {
		setValue(P_OUT_DATE, value);
	}

	/**
	 * Get the <code>p_out_number</code> parameter OUT value from the routine
	 */
	public Integer getPOutNumber() {
		return getValue(P_OUT_NUMBER);
	}

	/**
	 * Get the <code>p_out_varchar</code> parameter OUT value from the routine
	 */
	public String getPOutVarchar() {
		return getValue(P_OUT_VARCHAR);
	}

	/**
	 * Get the <code>p_out_date</code> parameter OUT value from the routine
	 */
	public Date getPOutDate() {
		return getValue(P_OUT_DATE);
	}
}
