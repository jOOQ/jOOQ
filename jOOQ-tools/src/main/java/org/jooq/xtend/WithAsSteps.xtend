        ...
        for (type : #[ "String", "Name"]) {
            for (degree : (1 .. Constants::MAX_ROW_DEGREE)) {
                out.append('''
                     ...
                     */
                    «generatedMethod»
                    @Support({ DB2, FIREBIRD, H2, HSQLDB, MYSQL_8_0, ORACLE, POSTGRES, SQLSERVER, SYBASE, VERTICA })
    ...
    def generateWithImpl() {
        ...
        for (degree : (1 .. Constants::MAX_ROW_DEGREE)) {
            outWithMethod.append('''
                ...
                @Override
                public final WithImpl with(String a, «XXXn(degree, "String fieldAlias")») {
                    return with(DSL.name(a), Tools.names(new String[] { «XXXn(degree, "fieldAlias")» }));
                }
            ''');
        }
        ...
        for (degree : (1 .. Constants::MAX_ROW_DEGREE)) {
            outWithMethod.append('''
                ...
                @Override
                public final WithImpl with(Name a, «XXXn(degree, "Name fieldAlias")») {
                    return with(a, new Name[] { «XXXn(degree, "fieldAlias")» });
                }
            ''');
        }
    ...
    def generateNameFields() {
        ...
        for (degree : (1 .. Constants::MAX_ROW_DEGREE)) {
            outAPI.append('''
                ...
                «generatedMethod»
                @Support({ DB2, FIREBIRD, H2, HSQLDB, MYSQL_8_0, ORACLE, POSTGRES, SQLSERVER, SYBASE, VERTICA })
                DerivedColumnList«degree» fields(«XXXn(degree, "Name fieldName")»);
            ''');
        }
        ...
        for (degree : (1 .. Constants::MAX_ROW_DEGREE)) {
            outImpl.append('''
                ...
                «generatedMethod»
                @Override
                public final DerivedColumnListImpl fields(«XXXn(degree, "Name fieldName")») {
                    return fields(new Name[] { «XXXn(degree, "fieldName")» });
                }
            ''');
        }

        insert("org.jooq.Name", outAPI, "fields");
        insert("org.jooq.impl.AbstractName", outImpl, "fields");
    }
}