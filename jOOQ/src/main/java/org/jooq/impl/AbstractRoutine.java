...
public abstract class AbstractRoutine<T> extends AbstractQueryPart implements Routine<T>, AttachableInternal {
    ...
    private final int executeCallableStatement() {
        ...
        try {
            ...
            if (ctx.configuration().dialect().family() == SQLDialect.SQLSERVER)
                Utils.consumeResultSets(ctx);
            /* [/pro] */
    ...
    private final void toSQLEnd(RenderContext context) {
        /* [pro] */
        if (hasDefaultedParameters() && context.configuration().dialect().family() == ORACLE) {
            context.sql(";")
                   .formatIndentEnd()
                   .formatSeparator()
                   .keyword("end;");
