...
public class UUIDWrapperArrayBinding implements Binding<UUID[], UUIDWrapper[]> {
    ...
    public Converter<UUID[], UUIDWrapper[]> converter() {
        return new Converter<UUID[], UUIDWrapper[]>() {
            ...
            public UUIDWrapper[] from(UUID[] t) {
                return t == null ? null : Seq.of(t).map(UUIDWrapper::new).toArray(UUIDWrapper[]::new);
            }

            @Override
            public UUID[] to(UUIDWrapper[] u) {
                return u == null ? null : Seq.of(u).map(UUIDWrapper::wrapped).toArray(UUID[]::new);
            }
    ...
    public void get(BindingGetResultSetContext<UUIDWrapper[]> ctx) throws SQLException {
        ctx.convert(converter()).value((UUID[]) (ctx.resultSet().getObject(ctx.index())));
    }
