/**
 * This class is generated by jOOQ
 */
package org.jooq.test.hsqldb.generatedclasses.tables;


import java.util.Arrays;
import java.util.List;

import org.jooq.Field;
import org.jooq.Table;
import org.jooq.TableField;
import org.jooq.UniqueKey;
import org.jooq.impl.TableImpl;
import org.jooq.test.hsqldb.generatedclasses.Keys;
import org.jooq.test.hsqldb.generatedclasses.Public;
import org.jooq.test.hsqldb.generatedclasses.tables.records.TTriggersRecord;


/**
 * This class is generated by jOOQ.
 */
@java.lang.SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class TTriggers extends TableImpl<TTriggersRecord> {

	private static final long serialVersionUID = -202861258;

	/**
	 * The reference instance of <code>PUBLIC.T_TRIGGERS</code>
	 */
	public static final TTriggers T_TRIGGERS = new TTriggers();

	/**
	 * The class holding records for this type
	 */
	@Override
	public java.lang.Class<TTriggersRecord> getRecordType() {
		return TTriggersRecord.class;
	}

	/**
	 * The column <code>PUBLIC.T_TRIGGERS.ID_GENERATED</code>.
	 */
	public final TableField<TTriggersRecord, java.lang.Integer> ID_GENERATED = createField("ID_GENERATED", org.jooq.impl.SQLDataType.INTEGER.nullable(false), this, "");

	/**
	 * The column <code>PUBLIC.T_TRIGGERS.ID</code>.
	 */
	public final TableField<TTriggersRecord, java.lang.Integer> ID = createField("ID", org.jooq.impl.SQLDataType.INTEGER, this, "");

	/**
	 * The column <code>PUBLIC.T_TRIGGERS.COUNTER</code>.
	 */
	public final TableField<TTriggersRecord, java.lang.Integer> COUNTER = createField("COUNTER", org.jooq.impl.SQLDataType.INTEGER, this, "");

	/**
	 * Create a <code>PUBLIC.T_TRIGGERS</code> table reference
	 */
	public TTriggers() {
		this("T_TRIGGERS", null);
	}

	/**
	 * Create an aliased <code>PUBLIC.T_TRIGGERS</code> table reference
	 */
	public TTriggers(java.lang.String alias) {
		this(alias, T_TRIGGERS);
	}

	private TTriggers(java.lang.String alias, Table<TTriggersRecord> aliased) {
		this(alias, aliased, null);
	}

	private TTriggers(java.lang.String alias, Table<TTriggersRecord> aliased, Field<?>[] parameters) {
		super(alias, Public.PUBLIC, aliased, parameters, "");
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public UniqueKey<TTriggersRecord> getPrimaryKey() {
		return Keys.PK_T_TRIGGERS;
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public List<UniqueKey<TTriggersRecord>> getKeys() {
		return Arrays.<UniqueKey<TTriggersRecord>>asList(Keys.PK_T_TRIGGERS);
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public TTriggers as(java.lang.String alias) {
		return new TTriggers(alias, this);
	}

	/**
	 * Rename this table
	 */
	public TTriggers rename(java.lang.String name) {
		return new TTriggers(name, null);
	}
}
