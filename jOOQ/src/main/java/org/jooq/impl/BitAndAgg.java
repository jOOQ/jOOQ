    ...
    MBitAndAgg<T>
{
    ...
    @Override
    public final MBitAndAgg<T> $value(MField<T> newValue) {
        return constructor().apply(newValue);
    }

    public final Function1<? super MField<T>, ? extends MBitAndAgg<T>> constructor() {
        return (a1) -> new BitAndAgg<>((Field<T>) a1);
    }
    ...
    public boolean equals(Object that) {
        if (that instanceof BitAndAgg) {
            return
                StringUtils.equals($value(), ((BitAndAgg) that).$value())
            ;
        }
        else
