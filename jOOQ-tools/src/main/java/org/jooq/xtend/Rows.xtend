    ...
    def generateRowClasses() {
        for (degree : (0..Constants::MAX_ROW_DEGREE)) {
            ...
            «ENDIF»
                 ...
                 * @see DSL#any(Object...)
                 */
                @Support({ ASE, CUBRID, DB2, DERBY, FIREBIRD, H2, HANA, HSQLDB, INGRES, MARIADB, MYSQL, ORACLE, POSTGRES, SQLSERVER, SYBASE, VERTICA })
...
«««                Condition isNotDistinctFrom(«Field_TN_tn(degree)»);
    ...
    final class RowImpl<«TN(Constants::MAX_ROW_DEGREE)»> extends AbstractQueryPart
    implements
