/**
 * This class is generated by jOOQ
 */
package org.jooq.example.db.oracle.mdsys.packages.sdo_geom;

/**
 * This class is generated by jOOQ.
 */
@javax.annotation.Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.6.0"
	},
	comments = "This class is generated by jOOQ"
)
@java.lang.SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class WithinDistance1 extends org.jooq.impl.AbstractRoutine<java.lang.String> {

	private static final long serialVersionUID = 483210885;

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.RETURN_VALUE</code>.
	 */
	public static final org.jooq.Parameter<java.lang.String> RETURN_VALUE = createParameter("RETURN_VALUE", org.jooq.impl.SQLDataType.VARCHAR, false);

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.GEOM1</code>.
	 */
	public static final org.jooq.Parameter<org.jooq.example.db.oracle.mdsys.udt.records.SdoGeometryRecord> GEOM1 = createParameter("GEOM1", org.jooq.example.db.oracle.mdsys.udt.SdoGeometry.SDO_GEOMETRY.getDataType(), false);

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.DIM1</code>.
	 */
	public static final org.jooq.Parameter<java.lang.Object> DIM1 = createParameter("DIM1", org.jooq.impl.DefaultDataType.getDefaultDataType("VARRAY"), false);

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.DIST</code>.
	 */
	public static final org.jooq.Parameter<java.math.BigDecimal> DIST = createParameter("DIST", org.jooq.impl.SQLDataType.NUMERIC, false);

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.GEOM2</code>.
	 */
	public static final org.jooq.Parameter<org.jooq.example.db.oracle.mdsys.udt.records.SdoGeometryRecord> GEOM2 = createParameter("GEOM2", org.jooq.example.db.oracle.mdsys.udt.SdoGeometry.SDO_GEOMETRY.getDataType(), false);

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.DIM2</code>.
	 */
	public static final org.jooq.Parameter<java.lang.Object> DIM2 = createParameter("DIM2", org.jooq.impl.DefaultDataType.getDefaultDataType("VARRAY"), false);

	/**
	 * The parameter <code>MDSYS.SDO_GEOM.WITHIN_DISTANCE.UNITS</code>.
	 */
	public static final org.jooq.Parameter<java.lang.String> UNITS = createParameter("UNITS", org.jooq.impl.SQLDataType.VARCHAR.defaulted(true), true);

	/**
	 * Create a new routine call instance
	 */
	public WithinDistance1() {
		super("WITHIN_DISTANCE", org.jooq.example.db.oracle.mdsys.Mdsys.MDSYS, org.jooq.example.db.oracle.mdsys.packages.SdoGeom.SDO_GEOM, org.jooq.impl.SQLDataType.VARCHAR);

		setReturnParameter(RETURN_VALUE);
		addInParameter(GEOM1);
		addInParameter(DIM1);
		addInParameter(DIST);
		addInParameter(GEOM2);
		addInParameter(DIM2);
		addInParameter(UNITS);
		setOverloaded(true);
	}

	/**
	 * Set the <code>GEOM1</code> parameter IN value to the routine
	 */
	public void setGeom1(org.jooq.example.db.oracle.mdsys.udt.records.SdoGeometryRecord value) {
		setValue(org.jooq.example.db.oracle.mdsys.packages.sdo_geom.WithinDistance1.GEOM1, value);
	}

	/**
	 * Set the <code>GEOM1</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setGeom1(org.jooq.Field<org.jooq.example.db.oracle.mdsys.udt.records.SdoGeometryRecord> field) {
		setField(GEOM1, field);
	}

	/**
	 * Set the <code>DIM1</code> parameter IN value to the routine
	 */
	public void setDim1(java.lang.Object value) {
		setValue(org.jooq.example.db.oracle.mdsys.packages.sdo_geom.WithinDistance1.DIM1, value);
	}

	/**
	 * Set the <code>DIM1</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setDim1(org.jooq.Field<java.lang.Object> field) {
		setField(DIM1, field);
	}

	/**
	 * Set the <code>DIST</code> parameter IN value to the routine
	 */
	public void setDist(java.lang.Number value) {
		setNumber(org.jooq.example.db.oracle.mdsys.packages.sdo_geom.WithinDistance1.DIST, value);
	}

	/**
	 * Set the <code>DIST</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setDist(org.jooq.Field<? extends java.lang.Number> field) {
		setNumber(DIST, field);
	}

	/**
	 * Set the <code>GEOM2</code> parameter IN value to the routine
	 */
	public void setGeom2(org.jooq.example.db.oracle.mdsys.udt.records.SdoGeometryRecord value) {
		setValue(org.jooq.example.db.oracle.mdsys.packages.sdo_geom.WithinDistance1.GEOM2, value);
	}

	/**
	 * Set the <code>GEOM2</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setGeom2(org.jooq.Field<org.jooq.example.db.oracle.mdsys.udt.records.SdoGeometryRecord> field) {
		setField(GEOM2, field);
	}

	/**
	 * Set the <code>DIM2</code> parameter IN value to the routine
	 */
	public void setDim2(java.lang.Object value) {
		setValue(org.jooq.example.db.oracle.mdsys.packages.sdo_geom.WithinDistance1.DIM2, value);
	}

	/**
	 * Set the <code>DIM2</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setDim2(org.jooq.Field<java.lang.Object> field) {
		setField(DIM2, field);
	}

	/**
	 * Set the <code>UNITS</code> parameter IN value to the routine
	 */
	public void setUnits(java.lang.String value) {
		setValue(org.jooq.example.db.oracle.mdsys.packages.sdo_geom.WithinDistance1.UNITS, value);
	}

	/**
	 * Set the <code>UNITS</code> parameter to the function to be used with a {@link org.jooq.Select} statement
	 */
	public void setUnits(org.jooq.Field<java.lang.String> field) {
		setField(UNITS, field);
	}
}
