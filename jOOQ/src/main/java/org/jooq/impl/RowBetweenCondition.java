...
implements
    ...
    private final QueryPartInternal delegate(Configuration configuration) {
        ...
        if (symmetric && asList(ASE, CUBRID, DB2, DERBY, FIREBIRD, H2, INGRES, MARIADB, MYSQL, ORACLE, SQLITE, SQLSERVER, SYBASE).contains(configuration.dialect().family())) {
            if (not) {
        ...
        else if (row.size() > 1 && asList(CUBRID, DERBY, FIREBIRD, INGRES, MARIADB, MYSQL, ORACLE, SQLITE, SQLSERVER, SYBASE).contains(configuration.dialect().family())) {
